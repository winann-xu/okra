...
- 阶段：阶段 3 —— M2 已收口（App/ 全部实现，核心链路端到端验证通过），下一步启动 M3（设置/一键还原/卸载）
- 更新时间：2026-09-18
- 下一步：M3：① SettingsView 补全（更新周期可配、镜像源选择、一键还原、卸载入口，均走 root helper 授权调用）；② helper 增补 restore/uninstall/设置读写命令（或扩展现有命令）；③ 还原后验证"其余 hosts 内容逐字节保留"（任务书 §9 M3 验收）；④ [立即更新] 授权弹窗真实环境实测（M2 遗留）。
